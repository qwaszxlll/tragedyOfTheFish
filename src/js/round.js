...
//Display Variables
var numPlayers = server.numPlayers; //Display Constant
...
function update () {
    ...
    if (inactive){
        ...
        if (idleTime >= idleLimit){
            inactive=false;
            server.addFinished(playerID, caught);
...
function createFishies(upperLimit, startY){
    var max = Math.min(Math.round(server.population/fishScale), maxFishShown);
...
// Handle Fish Catching Animations and Calculations
    ...
    .onComplete.add(function(){
        ...
        caught += haul;
        server.addCatch(haul); //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!create server function to deal with negative values!!!!!!!!!!!!!!!
        caughtFish.destroy();
        catchText.destroy();
    })
}
